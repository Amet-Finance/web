...
export default function MainDetailsContainer({bondDetailed}: { bondDetailed: ContractExtendedFormat }) {
    ...
    const issuanceDateClean = `${issuanceDateInFormat.toLocaleTimeString('en-GB', {
        hour: '2-digit',
        minute: '2-digit'
    })} ${issuanceDateInFormat.toLocaleDateString()}`.replace(/\//g, '.');
                ...
                <div className='flex justify-between items-center w-full'>
                    <div className='flex gap-2'>
                        ...
                        <div className='flex flex-col'>
                            ...
                            <span className='font-thin text-neutral-400 text-sm'>{investment.symbol} - {interest.symbol}</span>
                        </div>
                        <span className='bg-neutral-900 h-min px-3 py-1 rounded-full text-neutral-400'>ZCB</span>
                    </div>
                    <div className='flex flex-col items-end'>
                        <div className='flex flex-col '>
                            <div className='grid grid-cols-3 border border-neutral-900 rounded-t-md'>
                                <div
                                    className='flex flex-col items-center border-r-2 px-3 py-1 border-neutral-900 cursor-pointer  rounded-tl-md '>
                                    <span className='text-md font-semibold'>{total}</span>
                                    <span className='text-xs text-neutral-400 font-light'>Total Bonds</span>
                                </div>
                                <div
                                    className='flex flex-col items-center border-r-2 px-3 py-1 border-neutral-900 cursor-pointer '>
                                    <span className='text-md font-semibold'>{purchased}</span>
                                    <span className='text-xs text-neutral-400 font-light'>Purchased</span>
                                </div>
                                <div
                                    className='flex flex-col items-center px-3 py-1 cursor-pointer rounded-tr-md'>
                                    <span className='text-md font-semibold'>{redeemed}</span>
                                    <span className='text-xs text-neutral-400 font-light'>Redeemed</span>
                                </div>
                                <div className='h-[2px] w-full bg-green-500 col-span-3'/>
                            </div>
                        </div>
